...
{
    public class GetConfirmedNurseHandler : IRequestHandler<GetConfirmedNurseQuery, ResultResponse<List<NurseRowDTO>>>
    {
        private readonly INuresRepositry nuresRepositry;
        private readonly IMapper mapper;

        public GetConfirmedNurseHandler(INuresRepositry nuresRepositry, IMapper mapper)
        {
            this.nuresRepositry = nuresRepositry;
            this.mapper = mapper;
        }
        public async Task<ResultResponse<List<NurseRowDTO>>> Handle(GetConfirmedNurseQuery request, CancellationToken cancellationToken)
        {
            var page = request.page;
            if (page < 1) page = 1;

            var spec = new NurseSpesfication(ConfrmationStatus.Approved);
